#!/usr/bin/env node
// Batch fetch venue photos ONLY for venues with upcoming events
// Skips venues that already have a hero image
//
// Usage: node scripts/batchVenuePhotos-upcoming.js
...
async function run() {
  ...
  // Get venue IDs that have upcoming events
  const now = new Date();
  const venueIds = await Event.distinct('venueRef', {
    date: { $gte: now },
    venueRef: { $ne: null }
  });
  console.log(`Found ${venueIds.length} venues with upcoming events`);

  // Only fetch for venues missing a hero image
  ...
  }).sort({ 'stats.upcomingEvents': -1 });

  console.log(`${venues.length} of those still need photos\n`);
  ...
  console.log(`Fetched: ${fetched} (Wikimedia: ${sources.wikimedia || 0}, Google Places: ${sources.google_places || 0})`);
